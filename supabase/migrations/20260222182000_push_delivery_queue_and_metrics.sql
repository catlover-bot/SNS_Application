create table if not exists public.push_notification_jobs (
  ...
  constraint push_notification_jobs_status_check check (
    status in ('pending','processing','sent','partial','failed','cancelled')
  )
);
...
create policy "push_jobs_insert_own"
  on public.push_notification_jobs
  for insert
...
create table if not exists public.push_delivery_events (
  ...
  created_at timestamptz not null default now(),
  constraint push_delivery_events_event_type_check check (
    event_type in ('queued','sent','error','device_not_registered','open')
  )
);
...
create index if not exists push_delivery_events_notification_idx
...
create policy "push_delivery_events_insert_own"
  on public.push_delivery_events
  for insert
  with check (auth.uid() = user_id);
...
create policy "push_delivery_daily_metrics_select_own"
  on public.push_delivery_daily_metrics
  for select
  using (auth.uid() = user_id);
...
begin
  ...
  values (
    p_user_id,
    p_day,
    coalesce(nullif(trim(p_kind), ''), '__all__'),
...
grant execute on function public.push_delivery_bump_daily_metrics(uuid, date, text, integer, integer, integer, integer, integer)
